...
export default function ExtractionRules() {
  ...
  const fetchFields = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:8003/api/settings/fields');
  ...
  const addField = async (e) => {
    ...
    try {
      await fetch('http://localhost:8003/api/settings/fields', {
        method: 'POST',
  ...
  const deleteField = async (fieldName) => {
    try {
      await fetch(`http://localhost:8003/api/settings/fields/${encodeURIComponent(fieldName)}`, {
        method: 'DELETE'
      });
      fetchFields();
    } catch (e) {
