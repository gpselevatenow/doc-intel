...
const ExtractionResults = ({ type, data, docId, onFieldClick, isReprocessing, onReprocess, selectedField }) => {
  ...
  const fetchCustomFields = async () => {
    try {
      const res = await fetch(`http://127.0.0.1:8004/api/settings/fields/${encodeURIComponent(docId)}`);
  ...
  const addField = async (e) => {
    ...
    try {
      await fetch('http://127.0.0.1:8004/api/settings/fields', {
  ...
  const deleteField = async (fieldName) => {
    try {
      await fetch(`http://127.0.0.1:8004/api/settings/fields/${encodeURIComponent(docId)}/${encodeURIComponent(fieldName)}`, { method: 'DELETE' });
      fetchCustomFields();
    } catch (e) { console.error(e); }
  };
  ...
  const submitFeedback = async (action) => {
    try {
      await fetch('http://127.0.0.1:8004/api/feedback/rate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
